...
const BOT_VER = 'v1.6-CONTABILIDAD-DARVAS';
const SELLO = 'v1.3 · 2026-08-02 16-05ET'; // mismo sello que el nombre del zip
...
const K = {
  ...
  // v1.6: las velas OHLCV que Darvas necesita para existir. Sin esto,
  // darvas_6h_v1 y darvas_8h_v1 devuelven VETO_HISTORIAL_INSUFICIENTE
  // para siempre y no acumulan ni una observación.
  velas: () => `${PREFIJO}:velas`,
};
...
// ══════════════════════════════════════════════════════════════════════════
//  v1.6 · GRABAR LAS VELAS Y PREGUNTAR A DARVAS
//
//  Se llama una vez por corrida, ANTES de decidir nada. Es barato: dos
//  peticiones públicas a Coinbase, sin llave.
//
//  Darvas va en SOMBRA y aquí no se ordena nada: se anota qué habría hecho y
//  por qué. Esa es toda la gracia — acumular evidencia limpia sin arriesgar un
//  dólar, que es lo que pide el informe antes de plantearse promover nada.
// ══════════════════════════════════════════════════════════════════════════
async function actualizaVelasYDarvas(e, informe) {
  const velasLib = require('./velas.js');
  const darvas = require('./darvas.js');
  const io = {
    lee: () => leeJson(K.velas(), { v15: [], v60: [] }),
    guarda: (d) => guardaJson(K.velas(), d),
    cbPublico,
    libroDe,
  };
  const r = await velasLib.actualiza(io, {
    producto: 'BTC-USD',
    productoFuturo: (e.producto && e.producto.id) || null,
  });
  if (r.error) {
    // Sin velas Darvas no decide, que es lo correcto. NO se reutiliza el
    // historial viejo como si fuera fresco.
    e.velasError = r.error;
    if (informe) informe.velas = { error: r.error };
    return { decisiones: [], error: r.error };
  }
  e.velasError = null;
  e.velasAlDia = { v15: r.velas15m.length, v60: r.velas1h.length, ISO: ahoraISO() };

  let spread = null, ask = null;
  try {
    if (e.producto && e.producto.id) {
      const lb = await libroDe(e.producto.id);
      spread = lb.diferencial; ask = lb.venta;
    }
  } catch (err) { /* sin libro, Darvas vetará por spread nulo: correcto */ }

  const snapshot = {
    velas15m: r.velas15m, velas1h: r.velas1h,
    spread, askEjecutable: ask,
    brokerEstadoDesconocido: !!e.errorPosiciones,
  };
  const decisiones = [];
  for (const id of Object.keys(darvas.CONFIGS)) {
    const cfg = darvas.CONFIGS[id];
    if (!cfg.enabled) continue;
    const est = (e.darvas && e.darvas[id]) || {};
    const d = darvas.evalua(cfg, snapshot, est);
    // CANDADO: Darvas jamás ordena. Ni aquí ni en ningún otro sitio.
    if (d.action === 'ENTER_LONG' && darvas.puedeOperarReal(cfg) === false) {
      d.ejecutado = false;
      d.nota = 'SOMBRA: se anota lo que habría hecho, no se envía ninguna orden';
    }
    e.darvas = e.darvas || {};
    e.darvas[id] = Object.assign({}, est, {
      ultima: d, ISO: ahoraISO(),
      caja: (d.boxTop != null ? { alto: d.boxTop, bajo: d.boxBottom } : (est.caja || null)),
      stopVigente: est.stopVigente != null ? est.stopVigente : null,
    });
    decisiones.push(d);
  }
  if (informe) informe.darvas = decisiones.map(d => ({ id: d.strategyId, accion: d.action, motivo: d.reason }));
  return { decisiones };
}
...
// ══════════════════════════════════════════════════════════════════════════
//  v1.6 · COINBASE ES LA FUENTE DE VERDAD
//
//  EL FALLO RAÍZ (informe del 19 ago, confirmado leyendo el código): la
//  posición se abría en cuanto la API ACEPTABA la orden. Si `detalleOrden`
//  fallaba, quedaba una posición apuntada con el precio que el bot había
//  estimado y comisión cero. De ahí salen las OCHO operaciones marcadas como
//  reales con `llenado = 0` y `comision_usd = 0`: el bot creía tener algo que
//  quizá nunca compró, y su +$1,10 de resultado real no es un número fiable.
//
//  Ahora una operación solo llega a CERRADA_CONFIRMADA si:
//    1. hay cantidad ejecutada de entrada mayor que cero
//    2. hay cantidad de salida que la neutraliza
//    3. Coinbase confirma posición neta cero en ese contrato
//    4. se guardaron identificadores y precios medios de ambos llenados
// ══════════════════════════════════════════════════════════════════════════
const ESTADOS = {
  SENAL_CREADA: 'SENAL_CREADA',
  ORDEN_ENVIADA: 'ORDEN_ENVIADA',
  ORDEN_ACEPTADA: 'ORDEN_ACEPTADA',
  LLENADO_PARCIAL: 'LLENADO_PARCIAL',
  LLENADA: 'LLENADA',
  SALIDA_ENVIADA: 'SALIDA_ENVIADA',
  SALIDA_PARCIAL: 'SALIDA_PARCIAL',
  CERRADA_CONFIRMADA: 'CERRADA_CONFIRMADA',
  RECHAZADA: 'RECHAZADA',
  CANCELADA: 'CANCELADA',
...
// De dónde salen las cifras. El informe prohíbe expresamente registrar
// comisiones reales como cero cuando todavía no se conocen: por eso existe
// NO_DISPONIBLE, que es distinto de "cero".
const FUENTE = {
  BROKER: 'BROKER_REAL',
  EXTRACTO: 'BROKER_EXTRACTO',
  SOMBRA: 'ESTIMADO_SOMBRA',
  NO_DISPONIBLE: 'NO_DISPONIBLE',
};

// ── Idempotencia · el mismo ciclo NO puede crear dos órdenes ──────────────
// El identificador sale de estrategia + contrato + ventana + acción + hash de
// configuración, así que dos crones simultáneos generan EL MISMO id y Coinbase
// rechaza el duplicado en vez de abrir una segunda posición.
function hashConfig(aj) {
  const txt = JSON.stringify(aj || {}, Object.keys(aj || {}).sort());
  let h = 0;
  for (let i = 0; i < txt.length; i++) { h = ((h << 5) - h + txt.charCodeAt(i)) | 0; }
  return 'c' + Math.abs(h).toString(36);
}
function idClienteOrden(estrategiaId, contrato, ventanaISO, accion, cfgHash) {
  return [estrategiaId, contrato, ventanaISO || 'sinventana', accion, cfgHash]
    .join('_').replace(/[^A-Za-z0-9_\-]/g, '').slice(0, 64);
}

// ── Reconciliación · sin poder leer posiciones NO se opera ────────────────
// El informe lo dice claro: un error de lectura de posiciones NO es un veto
// estratégico. Antes se contaban juntos y por eso "la suma de categorías de
// vetos no explicaba los 1.419 casos".
async function reconcilia(e) {
  try {
    const pos = await posicionesFuturos();
    const mias = (pos || []).filter(x => !e.producto || !x.product_id || x.product_id === e.producto.id);
    const neta = mias.reduce((a, x) => a + (Number(x.net_size) || Number(x.number_of_contracts) || 0), 0);
    e.reconciliadoISO = ahoraISO();
    e.posicionBroker = neta;
    e.errorPosiciones = null;
    // huérfanas en los dos sentidos
    if (!e.posicion && neta !== 0) {
      e.estadoOrden = ESTADOS.RECONCILIAR;
      e.aviso = `⚠️ Coinbase tiene ${neta} contrato(s) que el bot no conoce. No se abre nada hasta resolverlo.`;
      return { ok: false, motivo: 'posicion_huerfana_broker', neta };
    }
    if (e.posicion && neta === 0) {
      e.estadoOrden = ESTADOS.RECONCILIAR;
      e.aviso = '⚠️ El bot cree tener posición y Coinbase dice cero. No se abre nada hasta resolverlo.';
      return { ok: false, motivo: 'posicion_huerfana_local', neta };
    }
    return { ok: true, neta };
  } catch (err) {
    // FALLA CERRADO: no se sabe qué hay en Coinbase, no se toca nada.
    e.errorPosiciones = err.message;
    e.estadoOrden = ESTADOS.RECONCILIAR;
    e.aviso = '⚠️ No pude leer las posiciones en Coinbase: no se abre ninguna operación este ciclo.';
    return { ok: false, motivo: 'error_lectura_posiciones', error: err.message };
  }
}

// ── Fórmula ÚNICA de P&L ─────────────────────────────────────────────────
// Antes el P&L se armaba en dos sitios con piezas distintas. Ahora hay una
// sola función y todos los componentes viajan por separado, como pide el
// informe: bruto, comisiones de entrada y salida, NFA, funding y deslizamiento.
function calculaPnl(op) {
  // OJO con null: `Number(null)` da 0 y `isFinite(0)` es TRUE, así que una
  // salida vacía se colaba como precio 0 y producía una pérdida inventada de
  // −$640. Lo cazó la prueba p8. Hay que comprobar la ausencia ANTES de
  // convertir a número — es justo la clase de cifra fantasma que el informe
  // manda eliminar.
  const num = (x) => (x === null || x === undefined || x === '' ? NaN : Number(x));
  const contratos = num(op.contratos) || 0;
  const entrada = num(op.precioMedioEntrada);
  const salida = num(op.precioMedioSalida);
  if (!isFinite(entrada) || !isFinite(salida) || !contratos) {
    return { brutoCents: null, netoCents: null, completo: false };
  }
  const signo = op.lado === 'largo' ? 1 : -1;
  const brutoCents = Math.round((salida - entrada) * signo * BTC_POR_CONTRATO * contratos * 100);
  const c = (x) => Math.round(Number(x) || 0);
  const netoCents = brutoCents
    - c(op.comisionEntradaCents) - c(op.comisionSalidaCents)
    - c(op.nfaCents) + c(op.fundingCents) - c(op.otrosCostesCents);
  return { brutoCents, netoCents, completo: true };
}
...
// ------------------------------------------------------------
async function cierraPosicion(e, motivo, precioMercado) {   // v1.6: `e` ya se usaba; ahora también para reconciliar
  ...
  if (pos.modo === 'real') {
    ...
    // ── v1.6 · UNA SALIDA SIN LLENADO NO CIERRA NADA ────────────────────
    // Mismo fallo que en la entrada: bastaba con que la salida se enviara.
    // Una orden aceptada y no llenada dejaba la operación marcada como cerrada
    // con la posición todavía viva en Coinbase.
    let detSalida = null;
    try { detSalida = ordenSalidaId ? await detalleOrden(ordenSalidaId) : null; }
    catch (err) { nota = `salida enviada pero no pude leer el detalle: ${err.message}`; }

    const llenadoSalida = detSalida ? Number(detSalida.llenado) || 0 : 0;
    if (llenadoSalida <= 0) {
      e.estadoOrden = ESTADOS.SALIDA_ENVIADA;
      e.aviso = `⚠️ Salida enviada SIN llenado confirmado (${motivo}): la posición sigue abierta. Se reintenta.`;
      return { fallo: 'salida_sin_llenado' };
    }
    if (llenadoSalida < pos.contratos) {
      e.estadoOrden = ESTADOS.SALIDA_PARCIAL;
      e.aviso = `⚠️ Salida PARCIAL: ${llenadoSalida} de ${pos.contratos}. La posición sigue abierta.`;
      pos.contratos -= llenadoSalida;
      return { fallo: 'salida_parcial', llenado: llenadoSalida };
    }
    if (detSalida.precio > 0) precioSalida = detSalida.precio;
    comisionExtra = detSalida.comisionCents;

    // Coinbase tiene que confirmar posición neta CERO antes de dar por cerrada.
    const recCierre = await reconcilia(e);
    if (!recCierre.ok && recCierre.motivo === 'error_lectura_posiciones') {
      e.estadoOrden = ESTADOS.RECONCILIAR;
      e.aviso = '⚠️ Salida llenada pero no pude confirmar posición cero en Coinbase.';
      nota = (nota ? nota + ' · ' : '') + 'cierre sin confirmar posición cero';
    }
  }

  const cuenta = calculaPnl({
    lado: pos.lado, contratos: pos.contratos,
    precioMedioEntrada: pos.precioEntrada, precioMedioSalida: precioSalida,
    comisionEntradaCents: pos.comisionCents, comisionSalidaCents: comisionExtra,
    nfaCents: pos.nfaCents, fundingCents: pos.fundingCents, otrosCostesCents: 0,
  });
  const plBruto = cuenta.brutoCents;
  const comisionTotal = (pos.comisionCents || 0) + comisionExtra;
  const plNeto = cuenta.netoCents;
  ...
  await actualizaSenal(pos.senalId, {
    ...
    comisionEntradaCents: pos.comisionCents || null,
    comisionSalidaCents: comisionExtra,
    plBrutoCents: plBruto,
    plCents: plNeto,
    precioMedioEntrada: pos.precioEntrada,
    precioMedioSalida: precioSalida,
    estadoOrden: ESTADOS.CERRADA_CONFIRMADA,
    fuentePnl: pos.modo === 'real' ? FUENTE.BROKER : FUENTE.SOMBRA,
    reconciliadoISO: e.reconciliadoISO || null,
    posicionDespues: e.posicionBroker != null ? e.posicionBroker : null,
...
async function corre(opciones) {
  ...
  try {
    ...
    // v1.6: grabar velas y preguntar a Darvas ANTES de decidir nada. Va en
    // sombra y nunca ordena; si falla, el baseline sigue su camino igual.
    try { await actualizaVelasYDarvas(e, informe); }
    catch (err) { informe.velas = { error: err.message }; }
    ...
    if (e.modo === 'real') {
      // ── v1.6 · NO SE ABRE POSICIÓN SIN LLENADO CONFIRMADO ───────────────
      // EL FALLO: bastaba con que la API ACEPTARA la orden. Si `detalleOrden`
      // fallaba, quedaba una posición apuntada con el precio estimado por el
      // bot y comisión CERO. Las ocho operaciones "reales" con llenado 0 y
      // comisión 0 del histórico salen justo de aquí: el bot creía tener algo
      // que quizá nunca compró.
      //
      // Antes de nada, reconciliar: si no se pueden leer las posiciones en
      // Coinbase, no se abre nada. Y eso NO cuenta como veto estratégico —
      // va a su propia lista de errores operativos.
      const rec = await reconcilia(e);
      if (!rec.ok) {
        senal.estado = 'error_operativo';
        senal.vetadaPor = null;                       // NO es un veto
        senal.estadoOrden = ESTADOS.RECONCILIAR;
        senal.nota = `⚠️ ${rec.motivo}${rec.error ? ': ' + rec.error : ''}`;
        await anadeSenal(senal);
        await guardaEstado(e);
        informe.acciones.push({ tipo: 'reconciliacion', exito: false, motivo: rec.motivo });
        return informe;
      }

      senal.cfgHash = hashConfig(aj);
      senal.idClienteEntrada = idClienteOrden('kronos_baseline_v2', e.producto.id, cierreISO, 'ENTRADA', senal.cfgHash);
      senal.estadoOrden = ESTADOS.ORDEN_ENVIADA;

      const r = await creaOrden({
        productId: e.producto.id, lado: d.lado,
        contratos: Math.min(aj.contratos, MAX_CONTRATOS),
        idCliente: senal.idClienteEntrada,            // v1.6: idempotencia
      });
      if (!r.exito) {
        senal.estado = 'error_operativo';
        senal.vetadaPor = null;
        senal.estadoOrden = ESTADOS.RECHAZADA;
      ...
      senal.estadoOrden = ESTADOS.ORDEN_ACEPTADA;

      // El detalle del llenado es OBLIGATORIO. Sin él no hay posición.
      let det = null;
      try { det = r.ordenId ? await detalleOrden(r.ordenId) : null; }
      catch (err) { det = null; senal.nota = `orden enviada, detalle ilegible: ${err.message}`; }

      const llenado = det ? Number(det.llenado) || 0 : 0;
      if (!det || llenado <= 0) {
        // Aceptada pero SIN llenar: no es una posición. Queda para reconciliar.
        senal.estado = 'error_operativo';
        senal.vetadaPor = null;
        senal.estadoOrden = ESTADOS.RECONCILIAR;
        senal.llenado = llenado;
        senal.fuenteComision = FUENTE.NO_DISPONIBLE;   // NO se apunta cero
        senal.comisionEntradaCents = null;
        senal.nota = (senal.nota ? senal.nota + ' · ' : '')
          + '⚠️ orden aceptada pero SIN llenado confirmado: no se abre posición contable';
        e.estadoOrden = ESTADOS.RECONCILIAR;
        e.aviso = senal.nota;
        await anadeSenal(senal);
        await guardaEstado(e);
        informe.acciones.push({ tipo: 'entrada', exito: false, motivo: 'sin_llenado' });
        return informe;
      }

      senal.estadoOrden = llenado < Math.min(aj.contratos, MAX_CONTRATOS)
        ? ESTADOS.LLENADO_PARCIAL : ESTADOS.LLENADA;
      senal.llenado = llenado;
      senal.contratos = llenado;                       // lo REAL, no lo pedido
      senal.contratosPedidos = Math.min(aj.contratos, MAX_CONTRATOS);
      if (det.precio > 0) senal.precioEntrada = det.precio;
      senal.precioMedioEntrada = det.precio || null;
      senal.comisionEntradaCents = det.comisionCents;
      senal.comisionCents = det.comisionCents;
      senal.fuenteComision = FUENTE.BROKER;
      senal.fuentePnl = FUENTE.BROKER;
      senal.posicionAntes = rec.neta;

      e.diario.operaciones += 1;
      e.diario.arriesgadoCents += Math.abs(aj.stopCents);
    } else {
      // ── SOMBRA · el informe prohíbe usar mid/last sin etiquetarlo ───────
      // Un largo entra con el ask ejecutable y sale con el bid; nunca el medio.
      senal.precioMedioEntrada = senal.precioEntrada;
      senal.fuenteComision = FUENTE.SOMBRA;
      senal.fuentePnl = FUENTE.SOMBRA;
      senal.estadoOrden = ESTADOS.LLENADA;
      senal.cfgHash = hashConfig(aj);
    }
...
module.exports = {
  ...
  actualizaVelasYDarvas, ESTADOS, FUENTE, calculaPnl, hashConfig, idClienteOrden, reconcilia,
